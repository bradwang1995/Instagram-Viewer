...
- Horizontal View keeps every card shell mounted and activates media across the visible width plus one width ahead. Each wheel event or Left/Right arrow press moves exactly one photo backward/forward and centers it when space permits. After the jump settles, the viewer retains one width behind and removes older iframe documents. At `1920 × 1080`, the media surface is `870px` high, leaving only a small controlled gap between media and the overlaid page chrome.
- Grid View remains four square columns on desktop and shows compact rows in the active viewport. Each wheel event or Up/Down arrow press moves exactly one row while preserving the selected column when possible. Iframe retention remains bounded to one viewport behind, the current viewport, and one viewport ahead.
...
- Horizontal uses ArrowLeft/ArrowRight for one-photo steps; Grid uses ArrowUp/ArrowDown for one-row steps. These browsing shortcuts apply across the archive surface, while orthogonal arrow keys are suppressed. When Slideshow is open, archive shortcuts are disabled and Slideshow exclusively owns ArrowLeft/ArrowRight navigation.
