...
- Horizontal View keeps every card shell mounted and activates media across the visible width plus one width ahead. Each wheel event or Left/Right arrow press targets exactly one photo backward/forward and reaches it with a smooth velocity-preserving transition, centering it when space permits. After the transition settles, the viewer retains one width behind and removes older iframe documents. At `1920 × 1080`, the media surface is `870px` high, leaving only a small controlled gap between media and the overlaid page chrome.
- Grid View remains four square columns on desktop and shows compact rows in the active viewport. Each wheel event or Up/Down arrow press targets exactly one row through the same smooth transition while preserving the selected column when possible. Iframe retention remains bounded to one viewport behind, the current viewport, and one viewport ahead.
...
- Horizontal uses ArrowLeft/ArrowRight for smooth one-photo steps; Grid uses ArrowUp/ArrowDown for smooth one-row steps. Consecutive inputs add exactly one target step while preserving the running transition velocity. These browsing shortcuts apply across the archive surface, while orthogonal arrow keys are suppressed. When Slideshow is open, archive shortcuts are disabled and Slideshow exclusively owns ArrowLeft/ArrowRight navigation.
- Slideshow starts automatically, loops the complete filtered queue, and defaults to five seconds per frame with an inline `3–10s` duration control. The previous and next media documents are mounted before they become current, so an existing preloaded iframe slides/crossfades into place instead of beginning its request at transition time. The implemented transition presets remain selectable in a compact dark control. Resolved photos use a same-image blurred full-viewport backdrop with a contained foreground image; compatibility iframes are non-interactive and visually cropped to mask Instagram chrome.
