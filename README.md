...
Every archive item keeps a lightweight card shell in the document. Horizontal activates the current viewport plus one viewport to the right; Grid doubles its forward loading distance to two viewports below so a two-row scroll burst has its whole four-column batch in flight before it arrives. Once scrolling settles, both views keep a one-viewport return cache, while Grid retains its additional second viewport ahead. Nearby reverse scrolling therefore stays instant without accumulating iframe documents for the whole page session. Filtering still hides an activated iframe without destroying it.
...
- Compatibility frames remain behind a near-black shield until their document load event, then crossfade in after a short settling delay instead of exposing a white document flash.
