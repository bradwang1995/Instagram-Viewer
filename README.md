...
Compatibility requests use the same bounded preload window as the direct-photo experience. Horizontal View keeps the visible cards plus three neighbors on each side; desktop Grid View retains the previous row while keeping the visible and next rows mounted. Permitted compatibility frames load eagerly, with at most three iframe navigations in flight, so nearby photos warm progressively without creating an unbounded iframe burst.
...
- Grid View is four square columns on desktop and shows two compact rows in the active viewport. A four-row bounded window retains one recently viewed row while keeping visible and ahead-of-scroll content mounted, so a large desktop library keeps no more than sixteen cards in the DOM; mobile keeps at most three.
...
Direct photo assets are preloaded ahead of the current viewport and retained in a small decoded-image window. Supported browsers also register a local cache-first service worker for image requests, so revisiting a previously loaded direct photo does not require the viewer to start from zero. Desktop Grid View mounts at most four four-photo rows, including one retained previous row; Horizontal View retains the visible photos plus three-photo overscan on each side. Compatibility iframe permission is limited to visible cards plus the next three items, those permitted frames use eager browser loading, and three active navigations are allowed at most.

Instagram compatibility previews are cross-origin iframe documents. The viewer keeps nearby and recently viewed previews mounted inside the bounded window to avoid unnecessary remounts, but it cannot place Instagram's iframe document itself into the app's image cache or control Instagram's own cache headers. Returning after a preview has left that bounded window can therefore still require an Instagram reload.
